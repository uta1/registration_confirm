...
def valid(data: dict) -> bool:
    return len(data) == 3 and ('username' in data) and ('password' in data) and ('email' in data)
...
def send_validation_code(username: str, email: str, password_hash: str) -> bool:
    try:
        ...
        smtpObj.starttls()
        smtpObj.login('backend.mipt@bk.ru','lolkek123')
        smtpObj.sendmail("backend.mipt@bk.ru", email, hash_str(username+email+password_hash))
        smtpObj.quit()
        return True
    except Exception:
        return False
...
def callback(ch, method, properties, body):
    ...
    if not prepare_registration(data['username'], data['email'], password_hash):
        print("registration of user {} failed on preparing".format(data['username']))
        return
    
    if not send_validation_code(data['username'], data['email'], password_hash):
